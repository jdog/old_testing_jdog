...
var PAGE = (function() {
	...
	*/
	...
	var Page = function(){}                // base constructor
		...
		, speedOfInterval = 100              // speed of interval
	...
	// respository of processes
	var procs = []
	, isRunning = false

	dog.runProcs = function() {
		if (procs.length) {
			var func = procs.shift()
	...
	* production code should be loading as minified unified code, not here, unless there is a good reason */
	var loadScript = dog.loadScript = function(/* pathToFile, allowCache, document */) {
		var map = mapArguments(arguments)
		, allowCache = map.Boo ? map.Boo[0] : false
		, doc = map.HTMLDocument ? map.HTMLDocument[0] : document
		...
		var scriptId = pathToFile.replace(/\./g,"_").replace(/\//g, "_").replace(":","")
			, existingElm = doc.getElementById(scriptId)
	...
	* }) */
	dog.extend = function extend(callback) {
		typeof callback === "function" && callback(puppy, dog, console)
		return puppy
	}
	...
	*/
	var waitExists = dog.waitExists = function(path, base, func) {
		...
		dog.addProc(function() {
			...
			var fard = dog.exists(path, base)
			if (fard) {
				waitList[path] = true
				typeof func === "function" && func(fard)
				runFinishedCallbacks()
				return
			}
			else dog.addProc(arguments.callee)
		})
	}
